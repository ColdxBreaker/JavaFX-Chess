...
import javafx.application.Application; // Importing the javafx package to create the JavaFx Application
import javafx.event.EventHandler; // Using the EventHandler functions to handle mouse clicks
import javafx.scene.Scene; // Using the Scene along with the application to create the game and it's settings (Ex. Size of the board)
import javafx.scene.image.Image; // Using the images for the chess pieces to be placed on the board
import javafx.scene.input.MouseEvent; // Using the MouseEvent to find mouse clicks
import javafx.scene.layout.GridPane; // Using the GridPane to set the layout of the chess pieces
import javafx.scene.shape.Rectangle; // Using rectangles to created the checkered squares on the board
import javafx.stage.Stage; // Using the Stage along with the scene to create the application
import javafx.scene.paint.Color; // Using color to set the colors with the tiles(rectangles/squares)
import javafx.scene.media.Media; // Using media to play a sound when a chess piece move is complete
import java.io.File; // Using File to get a file stream which is used along with Media
import javafx.scene.media.MediaPlayer; // Using the Media player to play the media
import javafx.util.Duration; // Using duration to reset the media back to the start to be played again

public class Game extends Application {
    private boolean isBlackTurn = false; // The boolean that will determine if it's white or black's turn
    private Board logicBoard = new Board(8, 8); // This sets the board size which will then later be used to check for patterns in the chess board (Internally)
    private ImageBoard chessImageBoard = new ImageBoard(8, 8); // This creates an imageboard that holds the chess piece images which can be accessed later(Internally)
    ...
    private String musicFile = "C:\\Users\\Chris (Local)\\Documents\\workspace\\CompSci_Chris\\src\\TheNewICS4UR\\Summative\\Sounds\\chessSound.mp3"; // The location of where the music file is stored
    ...
    public void start(Stage mainStage) {
        ...
        mainStage.getIcons().add(new Image("file:" + "C:\\Users\\Chris (Local)\\Documents\\workspace\\CompSci_Chris\\src\\TheNewICS4UR\\Summative\\Chess\\PawnB.png")); // Setting the icon of the application
    ...
    public boolean isKingCheck(int translatedX, int translatedY) {
        ...
        Piece chessPiece = logicBoard.chessBoard[translatedY][translatedX];
        // Rewrite
        try {
    ...
    private void onClick(int originalMouseX, int originalMouseY, int translatedMouseX, int translatedMouseY) {
        ...
        Piece chessPiece = logicBoard.getPiece(originalMouseY, originalMouseX); // Obtaining the chess piece on the gameboard
                ...
                switch (chessPiece.getChessPieceName()) { // A switch case based on the chess piece name
                    case "Pawn":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            boolean singleSpaceMove = ((Pawn) chessPiece).canMoveSingleSpace(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, isBlackTurn, chessImageBoard.imgBoard);
                            boolean diagonalSingleMove = ((Pawn) chessPiece).canMoveDiagonal(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, isBlackTurn, chessImageBoard.imgBoard, logicBoard.chessBoard);
                    ...
                    case "Rook":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            ...
                            if (((Rook) chessPiece).canMoveVertical(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, logicBoard.chessBoard)) {
                                try {
                    ...
                    case "Knight":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            ...
                            if (((Knight) chessPiece).canMoveInLShape(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY    )) {
                                try {
                    ...
                    case "Bishop":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            ...
                            if (((Bishop) chessPiece).canMoveDiagonal(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, logicBoard.chessBoard)) {
                                try {
                    ...
                    case "King":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            ...
                            if (((King) chessPiece).canMove(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, logicBoard.chessBoard)) {
                                {
                                    Piece originalPiece = logicBoard.chessBoard[originalMouseY][originalMouseX]; // Checking if the king can move in that position, if not then these will be kept as constants to revert the temporary change
                                    Piece translatedPiece = logicBoard.chessBoard[translatedMouseY][translatedMouseX]; // Checking if the king can move in that position, if not then these will be kept as constants to revert temporary change
                    ...
                    case "Queen":
                        if (chessPiece.isBlack() == isBlackTurn) { // Checking whether it's White or Black's turn
                            ...
                            if (((Queen) chessPiece).canMoveVertical(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, logicBoard.chessBoard) || ((Queen) chessPiece).canMoveDiagonal(originalMouseX, originalMouseY, translatedMouseX, translatedMouseY, logicBoard.chessBoard)) {
                                {
                                    try {
    ...
    private void defaultChessPosition() {
        ...
        for (int i = 0; i <= 7; i++) {
            logicBoard.addPiece(new Pawn(false), 6, i); // Added the Pawn to the game board board (internal calculations)
            logicBoard.addPiece(new Pawn(true), 1, i); // Added the Pawn to the game board board (internal calculations)
        ...
        for (int i = 0; i <= 7; i += 7) {
            logicBoard.addPiece(new Rook(false), 7, i); // Added the Rook to the game board board (internal calculations)
            logicBoard.addPiece(new Rook(true), 0, i); // Added the Rook to the game board board (internal calculations)
        ...
        for (int i = 1; i <= 6; i += 5) {
            logicBoard.addPiece(new Knight(false), 7, i); // Added the Knight to the game board board (internal calculations)
            logicBoard.addPiece(new Knight(true), 0, i); // Added the Knight to the game board board (internal calculations)
        ...
        for (int i = 2; i <= 5; i += 3) {
            logicBoard.addPiece(new Bishop(false), 7, i); // Added the Bishop to the game board board (internal calculations)
            logicBoard.addPiece(new Bishop(true), 0, i); // Added the Bishop to the game board board (internal calculations)
        ...
        logicBoard.addPiece(new King(false), 7, 4); // Added the King to the game board board (internal calculations)
        logicBoard.addPiece(new King(true), 0, 4); // Added the King to the game board board (internal calculations)
        ...
        logicBoard.addPiece(new Queen(false, Queen.NAME), 7, 3); // Added the Queen to the game board board (internal calculations)
        logicBoard.addPiece(new Queen(true, Queen.NAME), 0, 3); // Added the Queen to the game board board (internal calculations)
